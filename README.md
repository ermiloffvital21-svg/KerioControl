...
- В разных сборках KerioControl могут отличаться названия методов и схема ответа API. Клиент теперь пробует несколько вариантов вызова `Logs.get` (named/positional параметры, `http` и `http_access`) и несколько форматов массива записей (`result` как массив, а также `items`, `list`, `entries`, `data`, `logs`, `records`).
