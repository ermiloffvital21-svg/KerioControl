...
- В разных сборках KerioControl могут отличаться названия методов и схема ответа API. Клиент теперь пробует несколько вариантов (`http` и `http_access`) и несколько форматов массива записей (`items`, `list`, `entries`, `data`).
